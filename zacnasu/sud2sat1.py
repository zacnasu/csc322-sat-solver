...
if __name__ == '__main__':
    ...
    for file_num in range(1, 96):
        puzzle_str = sys.stdin.readline().replace(" ", "").replace("\n", "")
        print(puzzle_str)
        ...
        puzzle_clauses = clauses.copy()
        print(len(puzzle_clauses))
        for i in range(0, 9):
            for j in range(0, 9):
                d = puzzle[i][j]
                # For each digit already known, a clause (with one literal). 
                if d != 0:
                    puzzle_clauses.append([value(i+1,j+1, d)])
        
        outputStr = "p cnf " + "729 " + str(len(puzzle_clauses)) + "\n"
        print(len(puzzle_clauses))
        for i, clause in enumerate(puzzle_clauses):
            for j, num in enumerate(clause):
                if j != len(clause) -1:
                    outputStr += str(num) + " "
                else:
                    outputStr += str(num) + " 0"
            if i != len(puzzle_clauses)-1:
                outputStr += "\n"
        f = open('top'+str(file_num)+'.cnf', 'w')
        f.write(outputStr)
        f.close()
